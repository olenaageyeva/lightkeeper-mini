...
export const News = () => {
    ...
    return <aside className="flex flex-col m-4 md:m-0 basis-1/2 relative divide-y divide-gray-600 p-4 bg-slate-50 rounded-md shadow animate-fadein">
        <h3 className="text-xl font-semibold">News</h3>
        <section>
            {news.slice(0, 5).map((item: NewsType) => <div key={item.headline} className="flex-col md:flex  md:flex-row my-2 md:m-4 bg-gray-100 rounded-md hover:bg-slate-200 shadow animate-fadein">
